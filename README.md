# Hi there! 👋 I'm Faseeh Zafar

🎓 I'm a Data Science student at FAST National University, Islamabad.  
💻 Currently in my 2nd semester, learning core programming concepts with a focus on **C++**.  
🚀 Passionate about solving problems, writing clean code, and exploring how technology can solve real-world challenges.  

---

## 🔧 What I'm Working On

- Building a strong foundation in **C++** and programming fundamentals  
- Doing practice work as part of my degree. 
- Exploring **GitHub** to understand version control and collaborative development  

---

## 📁 Repositories You'll Find Here

-  Programming Fundamentals lab tasks in C++  
-  Practice problems and logic-building exercises  
-  Early-stage projects aligned with my coursework  

---

## 🧠 Skills I'm Developing

- Programming with **C++**
- Basics of Python and Data Science
- Version control with **Git & GitHub**
- Problem-solving and algorithmic thinking

---

## 📌 Goals

- Improve coding logic and structure  
- Contribute to meaningful projects  
- Get comfortable with GitHub for real-world use  
- Prepare for future roles in data science and development  

---
## 🛠️ Languages and Tools:

<p align="left">
  <img src="https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white"/>
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white"/>
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white"/>
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white"/>
  <img src="https://img.shields.io/badge/MATLAB-0076A8?style=for-the-badge&logo=mathworks&logoColor=white"/>
</p>

> ## 📊 Top Languages
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs?username=FaseehZafar&show_icons=true&locale=en&layout=compact)

## 📫 Let's Connect

I'm always open to learning, collaborating, or just chatting about tech!

> 💼 [LinkedIn]([https://www.linkedin.com](https://www.linkedin.com/in/faseeh-zafar-418369323/))   
> 💌 Email: faseehzafar.dev@gmail.com
---

_This GitHub profile reflects my learning journey — thanks for visiting!_
